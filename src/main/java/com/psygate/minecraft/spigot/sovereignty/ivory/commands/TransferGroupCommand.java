 ...
 */
public class TransferGroupCommand extends NucleusPlayerCommand {
    ...
    protected void subOnCommand(Player player, Command command, String s, String[] args) throws Exception {
        String name = args[0];
        try {
            UUID to = PlayerManager.getInstance().toUUID(args[1]);
            Optional<? extends Group> groupopt = GroupManager.getInstance().getGroup(name);
            if (!groupopt.isPresent()) {
                throw new CommandException("Group not found.");
            } else {
                Group group = groupopt.get();
                if (!group.getCreator().equals(player.getUniqueId()) && !player.isOp()) {
                    throw new CommandException("Insufficient permission.");
                } else {
                    group.setCreator(to);
                    group.removeMember(player.getUniqueId());
                    group.addMember(to, Rank.CREATOR, player.getUniqueId(), false);
                    player.sendMessage(ChatColor.GREEN + "Group transferred.");
                }
            }
    ...
    protected String[] getName() {
        return new String[]{"transfergroup"};
    }
}
