 ...
 */
public class UnignorePlayerCommand extends NucleusPlayerCommand {

    public UnignorePlayerCommand() {
        super(1, 1);
    }
    ...
    protected void subOnCommand(Player player, Command command, String s, String[] args) throws Exception {
        String name = args[0];
        try {
            UUID uuid = PlayerManager.getInstance().toUUID(name);
            GroupManager.getInstance().unignorePlayer(player.getUniqueId(), uuid);
            player.sendMessage(ChatColor.GREEN + "Player unignored.");
