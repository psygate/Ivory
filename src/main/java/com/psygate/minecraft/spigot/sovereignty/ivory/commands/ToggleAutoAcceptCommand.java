 ...
 */
public class ToggleAutoAcceptCommand extends NucleusPlayerCommand {

    public ToggleAutoAcceptCommand() {
        super(0, 0);
    }
    ...
    protected void subOnCommand(Player player, Command command, String s, String[] args) throws Exception {
        GroupManager.getInstance().togglePlayerAutoAccept(player.getUniqueId());
        boolean setting = GroupManager.getInstance().isAutoAcceptInvite(player.getUniqueId());
        player.sendMessage(ChatColor.GREEN + "Auto accept now: " + ((setting) ? ChatColor.GREEN : ChatColor.RED) + setting);
    }

    @Override
    protected String[] getName() {
        return new String[]{"toggleautoaccept"};
    }
}
