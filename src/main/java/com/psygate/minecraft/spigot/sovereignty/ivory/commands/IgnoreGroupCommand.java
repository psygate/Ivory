 ...
 */
public class IgnoreGroupCommand extends NucleusPlayerCommand {

    public IgnoreGroupCommand() {
        super(1, 1);
    }
    ...
    protected void subOnCommand(Player player, Command command, String s, String[] args) throws Exception {
        String name = args[0];
        GroupManager.getInstance().ignoreGroup(player.getUniqueId(), name);
        asyncMessage(player, ChatColor.GREEN + "Group ignored.");
    }
