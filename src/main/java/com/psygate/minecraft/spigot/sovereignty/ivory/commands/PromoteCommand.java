 ...
 */
public class PromoteCommand extends NucleusPlayerCommand {
    ...
    protected void subOnCommand(Player player, Command command, String s, String[] args) throws Exception {
        ...
        if (membername == player.getName()) {
            throw new CommandException("Cannot promote self.");
        }
        ...
        if (!groupopt.isPresent()) {
            ...
            } else if (group.getMembers().get(memuuid).getRank().ge(group.getMembers().get(player.getUniqueId()).getRank()) && !player.isOp()) {
            ...
            } else if (group.getMembers().get(memuuid).getRank().ge(newrank)) {
                throw new CommandException("Rank is less than current rank, use demote.");
            } else {
                group.getMembers().get(memuuid).setRank(newrank);
                player.sendMessage(ChatColor.GREEN + "Member promoted.");

                Player p = Bukkit.getPlayer(memuuid);
                if (p != null) {
                    p.sendMessage(ChatColor.GREEN + player.getName() + " promoted you to " + newrank + " in " + group.getName());
                }
            }
        }
    }
