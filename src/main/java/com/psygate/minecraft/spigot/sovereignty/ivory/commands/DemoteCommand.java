 ...
 */
public class DemoteCommand extends NucleusPlayerCommand {
    ...
    protected void subOnCommand(Player player, Command command, String s, String[] args) throws Exception {
        ...
        try {
            ...
            throw new CommandException("Invalid rank: " + args[2]);
        }

        if (membername == player.getName()) {
            throw new CommandException("Cannot demote self.");
        }
        ...
        if (!groupopt.isPresent()) {
            ...
            if (group.getCreator().equals(memuuid)) {
                throw new CommandException("Unable to demote or promote creator.");
            } else if ((!group.hasMemberWithRankGE(player.getUniqueId(), Rank.MODERATOR)
                    || !group.hasMemberWithRankGE(player.getUniqueId(), newrank)) && !player.isOp()) {
            ...
            } else if (group.getMembers().get(memuuid).getRank().gt(group.getMembers().get(player.getUniqueId()).getRank()) && !player.isOp()) {
            ...
            } else if (group.getMembers().get(memuuid).getRank().le(newrank)) {
                throw new CommandException("Rank is greater than current rank, use promote.");
            } else {
                group.getMembers().get(memuuid).setRank(newrank);
                player.sendMessage(ChatColor.GREEN + "Member demoted.");

                Player p = Bukkit.getPlayer(memuuid);
                if (p != null) {
                    p.sendMessage(ChatColor.YELLOW + player.getName() + " demoted you to " + newrank + " in " + group.getName());
                }
            }
        }
    }
